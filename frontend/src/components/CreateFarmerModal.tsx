...
export default function CreateFarmerModal() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        const newFarmer = { name, state, city, document, id: selectedObject?.id || Date.now() };
        if (isEdit) {
    ...
    return (
        <Modal
            isOpen={createFarmerMmodalIsOpen}
            onRequestClose={() => setCreateFarmerMmodalIsOpen(false)}
            style={{
                content: {
                    top: '50%',
                    left: '50%',
        ...
        >
            <button
                onClick={() => setCreateFarmerMmodalIsOpen(false)}
                style={{
                    position: 'absolute',
                    top: '10px',
                    right: '10px',
