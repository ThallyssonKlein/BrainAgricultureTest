...
class TestOutboundCultureRepositoryPort:
    ...
    async def test_delete_culture_query_validation_failed_with_rollback(self, culture_repository, mock_session, trace_id):
        culture_id = 1
    ...
    async def test_create_culture_for_a_farmer_query_validation_failed_with_rollback(self, culture_repository, mock_session, valid_culture_data, trace_id):
        farmer_id = 1

        mock_session.execute.side_effect = Exception("Error")
        with pytest.raises(Exception):
            await culture_repository.create_culture_for_a_farmer(farmer_id, valid_culture_data, trace_id)

        mock_session.rollback.assert_called_once()
    
    async def test_get_cultures_for_a_farmer_query_validation_failed_with_rollback(self, culture_repository, mock_session, trace_id):
        farmer_id = 1

        mock_session.execute.side_effect = Exception("Error")
        with pytest.raises(Exception):
            await culture_repository.get_cultures_for_a_farmer(farmer_id, trace_id)

        mock_session.rollback.assert_called_once()
    
    async def test_get_by_farmer_id_and_name_query_validation_failed_with_rollback(self, culture_repository, mock_session, trace_id):
        farmer_id = 1
        culture_name = "Wheat"

        mock_session.execute.side_effect = Exception("Error")
        with pytest.raises(Exception):
            await culture_repository.get_by_farmer_id_and_name(farmer_id, culture_name, trace_id)

        mock_session.rollback.assert_called_once()
    
    async def test_update_culture_by_id_query_validation_failed_with_rollback(self, culture_repository, mock_session, valid_culture_data, trace_id):
        culture_id = 1

        mock_session.execute.side_effect = Exception("Error")
        with pytest.raises(Exception):
            await culture_repository.update_culture_by_id(culture_id, valid_culture_data, trace_id)

        mock_session.rollback.assert_called_once()
    
    async def test_find_culture_by_id_query_validation_failed_with_rollback(self, culture_repository, mock_session, trace_id):
        culture_id = 1

        mock_session.execute.side_effect = Exception("Error")
        with pytest.raises(Exception):
            await culture_repository.find_culture_by_id(culture_id, trace_id)
